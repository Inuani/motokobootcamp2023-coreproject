...
import {
  embryo_dao as embryoDao,
  createActor as createEmbryoDaoActor,
  canisterId as embryoDaoCanisterId,
  idlFactory as embryoDaoIdlFactory,
} from "../declarations/embryo_dao";
...
type State = {
  ...
  embryoDaoActor: typeof embryoDao;
  principal: Principal;
  accountId: string;
  error: string;
  isLoading: boolean;
...
const defaultState: State = {
  isAuthed: null,
  embryoDaoActor: createEmbryoDaoActor(embryoDaoCanisterId, {
...
export const createStore = ({
  ...
  const initStoic = async (identity: Identity & { accounts(): string }) => {
    ...
    const counterActor = createEmbryoDaoActor(embryoDaoCanisterId, {
      agentOptions: {
        identity,
        host: HOST,
      },
  ...
  const initPlug = async () => {
    ...
    const embryoDaoActor = (await window.ic?.plug.createActor({
      canisterId: embryoDaoCanisterId,
      interfaceFactory: embryoDaoIdlFactory,
    })) as typeof embryoDao;
  ...
  interface Window {
    ic: {
      plug: {
        ...
        createActor: (options: {}) => Promise<typeof embryoDao>;
